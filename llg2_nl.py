...
E_series = TimeSeries('/home/mnv/Documents/python_doc/llg_nl/E_mid_5')
dEdz_series = TimeSeries('/home/mnv/Documents/python_doc/llg_nl/E_mid_5_dEdz')
...
E_old = 0
th = Constant(theta)
N_f = 10000 #300 #100000
n = FacetNormal(mesh)
oo = Constant(0)
PI = Constant(math.pi)
Hd_v_y = as_vector((oo, Constant(-26), oo))
#hd_s+hd_ext
F = dot(w,(v-m)/Dt-al*cross(m,(v-m)/Dt))*dx \
+ (1-th)**2*dot(w,cross(m,h_rest(m,pp,e_f,dedz_v,M_s*M_s/2/k*phi,M_s*M_s/2/k*(hd_ext + Hd_v_y))))*dx  + (1-th)*th*dot(w,cross(v,h_rest(m,pp,e_f,dedz_v,M_s*M_s/2/k*phi,M_s*M_s/2/k*(hd_ext + Hd_v_y))))*dx + (1-th)*th*dot(w,cross(m,h_rest(v,pp,e_f,dedz_v,M_s*M_s/2/k*phi,M_s*M_s/2/k*(hd_ext + Hd_v_y))))*dx + th**2*dot(w,cross(v,h_rest(v,pp,e_f,dedz_v,M_s*M_s/2/k*phi,M_s*M_s/2/k*(hd_ext + Hd_v_y))))*dx \
